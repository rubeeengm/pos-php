...
class Auth {
    ...
    private static function encryptCookie(string $value) : string {
        $method = 'AES-256-CBC';
        $key = hash('sha256', self::aud());
        $iv = substr(hash('sha256', self::aud()), 0, 16);
        
        return base64_encode(openssl_encrypt($value, $method, $key, 0, $iv));
    }
    ...
    private static function decryptCookie(string $value) : string {
        ...
        return openssl_decrypt(base64_decode($value), $method, $key, 0, $iv);
    }
